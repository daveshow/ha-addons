...
sync_delete="$(bashio::config 'sync_delete' 'false')"
monitor_path="/backup"
delete_command=""
...
aws s3 sync $monitor_path s3://"$bucket_name"/ --no-progress --region "$bucket_region" --storage-class "$storage_class" "$delete_command"
